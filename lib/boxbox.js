...
(function() {
    ...
    var World = {
        ...
        tick: function(delta) {
            ...
            // destroy
            for (i = 0; i < this._destroyQueue.length; i++) {
                toDestroy = this._destroyQueue.pop();
                id = toDestroy._id;
                world.DestroyBody(toDestroy._body);
                toDestroy._destroyed = true;
                delete this._keydownHandlers[id];
                delete this._startContactHandlers[id];
                delete this._finishContactHandlers[id];
                delete this._impactHandlers[id];
                this._destroyQueue.splice(id, 1);
                this._impulseQueue.splice(id, 1);
                delete this._constantVelocities[id];
                delete this._constantForces[id];
            ...
            // create
            for (i = 0; i < this._creationQueue.length; i++) {
                this.createEntity(self._creationQueue.pop());
            }

            // position
            for (i = 0; i < this._positionQueue.length; i++) {
                o = this._positionQueue.pop();
    ...
    var Entity = {
        ...
        _init: function(world, options, id) {
            ...
            body.linearDamping = ops.linearDamping;
            ...
            if (ops.userData) {
                this._body.SetUserData(ops.userData);
            }
        ...
        userData: function(value) {
            if (value !== undefined) {
                this._body.SetUserData(value);
            }
            return this._body.GetUserData();
        },
